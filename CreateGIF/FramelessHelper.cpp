...
void WidgetData::setMax(bool bMax)
{
    if(m_bMax != bMax)
    {
        if(bMax)
        {
			...
			//求当前窗口在哪个显示器上面积最多，那么最大化就充满那个显示器
			...
			{
				QRect rcScreen = screens.at(i)->availableGeometry();
				QRect rc = rcScreen.intersected(m_rect);
				int nAreaSize = rc.width() * rc.height();	
				if (nAreaSize > nMax)
				{
					nMax = nAreaSize;
					nIndex = i;
			...
			QRect rect = screens.at(nIndex)->availableGeometry();
